...
public class TextEditor extends EditText implements TextWatcher {
    ...
    private void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.TextEditor);
        bulletColor = array.getColor(R.styleable.TextEditor_bulletColor, 0);
        bulletRadius = array.getDimensionPixelSize(R.styleable.TextEditor_bulletRadius, 0);
        bulletGapWidth = array.getDimensionPixelSize(R.styleable.TextEditor_bulletGapWidth, 0);
        historyEnable = array.getBoolean(R.styleable.TextEditor_historyEnable, true);
        historySize = array.getInt(R.styleable.TextEditor_historySize, 100);
        linkColor = array.getColor(R.styleable.TextEditor_linkColor, 0);
        linkUnderline = array.getBoolean(R.styleable.TextEditor_linkUnderline, true);
        quoteColor = array.getColor(R.styleable.TextEditor_quoteColor, 0);
        quoteStripeWidth = array.getDimensionPixelSize(R.styleable.TextEditor_quoteStripeWidth, 0);
        quoteGapWidth = array.getDimensionPixelSize(R.styleable.TextEditor_quoteCapWidth, 0);
        array.recycle();
    ...
    protected void styleInvalid(int style, int start, int end) {
        ...
        List<SoftsPart> list = new ArrayList<>();

        for (StyleSpan span : spans) {
            if (span.getStyle() == style) {
                list.add(new SoftsPart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
                getEditableText().removeSpan(span);
            }
        }

        for (SoftsPart part : list) {
            if (part.isValid()) {
    ...
    protected void bulletValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (bulletStart < bulletEnd) {
                getEditableText().setSpan(new BulletSpannn(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void quoteValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (quoteStart < quoteEnd) {
                getEditableText().setSpan(new QuoteSpannn(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void linkValid(String link, int start, int end) {
        ...
        linkInvalid(start, end);
        getEditableText().setSpan(new UrlSpannn(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public void fromHtml(String source) {
        ...
        builder.append(SoftsParser.fromHtml(source));
        switchToKnifeStyle(builder, 0, builder.length());
        setText(builder);
    }

    public String toHtml() {
        return SoftsParser.toHtml(getEditableText());
    }
    ...
    protected void switchToKnifeStyle(Editable editable, int start, int end) {
        ...
        for (BulletSpan span : bulletSpans) {
            ...
            editable.setSpan(new BulletSpannn(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (QuoteSpan span : quoteSpans) {
            ...
            spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
            editable.removeSpan(span);
            editable.setSpan(new QuoteSpannn(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (URLSpan span : urlSpans) {
            ...
            int spanEnd = editable.getSpanEnd(span);
            editable.removeSpan(span);
            editable.setSpan(new UrlSpannn(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
}
